...
(function () {
    ...
    function rewriteConfigIni(text, locale) {
        ...
        // 游戏资源走 CDN 直连,不强制经本机反代(避免下载二次中转变慢)
        text = text.replace(
            /^(\s*gameresBaseUrl\s*=\s*).*$/m,
            "$1//wyhjres2.bun.sh.cn/"
        );
        text = text.replace(
            /^(\s*mapsBaseUrl\s*=\s*).*$/m,
            "$1" + base + "/maps/"
        ...
        // 资源包走高速 CDN,避免受本站出口带宽限制
        text = text.replace(
            /^(\s*gameResArchiveUrl\s*=\s*).*$/m,
            "$1https://download.ra2web.com/fully-music.exe"
        );
        return text;
    }
    ...
    function rewriteExternalUrl(url) {
        ...
        // 游戏资源 CDN 保持直连,不再改写为本站 /gameres/ 反代
        return url;
    }
