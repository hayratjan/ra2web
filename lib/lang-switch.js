...
(function () {
    ...
    function rewriteExternalUrl(url) {
        ...
        if (/^\/\/wyhjres\.bun\.sh\.cn\//i.test(url)) {
            return url.replace(/^\/\/wyhjres\.bun\.sh\.cn\//i, "/gameres/");
        }
        return url;
    }
            ...
            try {
                ...
                if (external !== url) {
                    if (isRequestObj && typeof Request === "function") {
                        return originalFetch(new Request(external, input), init);
                    }
                    return originalFetch(external, init);
                }
