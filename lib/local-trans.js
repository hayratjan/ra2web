...
ra2webInjection.translationMap = {
    ...
    'Ladder Season 2 is live. Play a "Quick Match" and get your rank badge now!':
        "这里是联机大厅，你可以创建或加入房间与玩家对战。排位赛请从主菜单选择「排位赛」。单机请返回主菜单选择「单机模式」。",
    ...
    "Join us on Discord: https://discord.gg/uavJ34JTWY":
        "欢迎加入 Discord 交流：https://discord.gg/yxkVn4wBad",
    您加入了美國指揮中心頻道:
        "您已返回房间大厅。",
    您已經與伺服器斷線了:
        "您已经与服务器断开连接，请检查网络后重试。",
    自訂戰役: "定制对局",
    基地重新部署: "基地可重新部署",
    升級工具箱: "随机宝箱",
    部隊數: "初始作战单位",
    ...
    "Set up a game automatically": "自动匹配对手，参与天梯排位",
    "Join a lobby to select an opponent": "进入联机大厅，创建或加入房间，支持多人实时对战",
...
/** 菜单按钮文案与 tooltip 补丁(CSF 内建中文不会走 locale 覆盖) */
ra2webInjection.menuLabelMap = {
    自定义房间: "联机大厅",
    自定義房間: "聯機大廳",
    "Custom Match": "联机大厅",
    自定义竞赛: "联机大厅",
...
ra2webInjection.menuTooltipMap = {
    "參與大廳，選擇對手": "进入联机大厅，创建或加入房间，支持多人对战",
    "参与大厅，选择对手": "进入联机大厅，创建或加入房间，支持多人对战",
    "Join a lobby to select an opponent":
        "进入联机大厅，创建或加入房间，支持多人对战",
};

ra2webInjection.patchMenuButtons = (root) => {
    if (!root || !root.querySelectorAll) {
        return;
    }
    root.querySelectorAll(".menu-button").forEach((btn) => {
        const label = (btn.textContent || "").trim();
        let mapped = ra2webInjection.menuLabelMap[label];
        // CSF 内建文案含「自定义房间」时做模糊匹配
        if (!mapped && /自定义房间|自定義房間|自定义竞赛|自訂競賽/i.test(label)) {
            mapped = "联机大厅";
        }
        if (mapped && label !== mapped) {
            btn.textContent = mapped;
        }
        const tip = btn.getAttribute("data-r-tooltip");
        if (tip) {
            const mappedTip =
                ra2webInjection.menuTooltipMap[tip] ||
                (/參與大廳|参与大厅|选择对手|選擇對手/i.test(tip)
                    ? "进入联机大厅，创建或加入房间，支持多人对战"
                    : null);
            if (mappedTip && tip !== mappedTip) {
                btn.setAttribute("data-r-tooltip", mappedTip);
            }
        }
    });
};
...
ra2webInjection.translateDOM = (node) => {
    if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        const textValue =
            ra2webInjection.translationMap[textContent] ||
            ra2webInjection.translationMap[trimmed];
        if (textValue) {
            ...
            } else if (ra2webInjection.containsOnlyTextOrIsEmpty(node)) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE || node.nodeType === Node.DOCUMENT_NODE) {
        ra2webInjection.patchMenuButtons(node);
    }
};

// 事件处理，挂载对 DOM 翻译与菜单补丁
document.addEventListener("DOMContentLoaded", () => {
    const runPatch = () => {
        ra2webInjection.translateDOM(document.body);
        ra2webInjection.patchMenuButtons(document.body);
    };
    ...
    observer.observe(document.body, {
        ...
        characterData: true,
        attributes: true,
        attributeFilter: ["data-r-tooltip"],
    });

    runPatch();
    // 主菜单由 React 反复渲染,持续轻量轮询确保文案不被 CSF 覆盖回去
    setInterval(runPatch, 2000);
});
...
// 额外样式从本站加载(不再请求 ra2web.github.io)
fetch("/style.css?v=0.65.1")
    .then((response) => {
        if (!response.ok) {
    ...
    .catch((error) => {
        console.error("Error fetching local CSS:", error);
    });
